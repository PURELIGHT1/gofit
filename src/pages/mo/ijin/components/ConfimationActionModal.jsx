...
import { useKonfirmasiIjin, useTolakIjin } from "../hooks/queries"

const ConfimationActionModal = ({ id, type, nama }) => {
    console.log(id);

    const { mutate: mutateTolak, isLoading: loadingTolak } = useTolakIjin();
     const { mutate: mutateKonfirmasi, isLoading: loadingKonfirmasi} = useKonfirmasiIjin();
                    ...
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-sm transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <div className="sm:flex sm:items-start">
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        <h3
                                            ...
                                            id="modal-title"
                                        >
                                        { type === 'tolak'? 'Tolak Ijin Instruktur' : 'Konfirmasi Ijin Instruktur'}{' '}
                                        </h3>
                                        <div className="mt-2">
                                            <p className="text-base text-gray-600">
                                                ...
                                                {type === 'tolak'
                                                    ? 'menolak'
                                                    : 'mengkonfirmasi'}{' '}
                                                ijin instruktur <strong>{nama}</strong>?
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                <button
                                    onClick= {
                                        type === 'tolak' ? onTolak : onKonfirmasi
                                        
                                    }
                                    disabled={loadingKonfirmasi || loadingTolak}
                                    type="button"
...
export default ConfimationActionModal;
