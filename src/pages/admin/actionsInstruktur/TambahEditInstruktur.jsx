...
let schema = yup.object().shape({
    ...
    inisial: yup.string().required('Inisial harus diisi'),
    alamat: yup.string().required('Alamat harus diisi'),
    tglLahir: yup.string().required('Tanggal Lahir harus diisi'), 
    // noHp: yup.string().required('Nomor HP harus diisi').matches(new RegExp('[0-9]{7}')),
...
const TambahEditInstruktur = ({ isEdit }) => {
    const { id } = useParams();
    const navigate = useNavigate();
    const { mutate: mutatePost, isLoading: loadingPost } = usePostInstruktur();
    const { mutate: mutatePut, isLoading: loadingPut } = usePutInstruktur();
    const { data, isLoading } = useInstrukturDetail(id);
    const {
        register,
        handleSubmit,
        formState: { errors },
...
export default TambahEditInstruktur;
